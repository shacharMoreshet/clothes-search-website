...
def write(x, img, phrases, coco_classes, colors):
    ...
    if cls == 0:  # for human bbox only detection
        ...
        for i, phrase in enumerate(phrases):
            cv2.putText(img, phrase, (int(c1[0]-200), int(c1[1] + t_size[1] + 150 + i*20)), cv2.FONT_HERSHEY_PLAIN, 1, color, 1 )
    return img
...
def write(x, img, raw_pharses,phrases, order, coco_classes, colors):
    ...
    if cls == 0:  # for human bbox only detection
        ...
        for i in range(len(attrs_name)):
            ...
            cv2.putText(img, label + " " + text, (int(c1[0] -t_size[0]), int(c1[1] + t_size[1] + 5 + i*t_size[1])), cv2.FONT_HERSHEY_PLAIN, 1,
                        [225, 255, 255], 1)
...
def view_image(bboxes):
    ...
    for bbox in bboxes:
        ...
        cv2.rectangle(img, (int(c1[0]), int(c1[1])), (int(c2[0]), int(c2[1])), 128+20, 3)
    cv2.imshow("01", img)
    cv2.waitKey(0)
